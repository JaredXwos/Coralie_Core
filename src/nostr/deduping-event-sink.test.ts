...
import { DedupingEventSink } from './deduping-event-sink'
...
describe('DedupingEventSink', () => {
  it('drops a duplicate event id arriving from a second relay', () => {
    const sink = new DedupingEventSink()
  ...
  it('accepts an event id again after it expires from the retention window', () => {
    ...
    const sink = new DedupingEventSink({ retentionWindowMs: 1000, now: clock.now })
    const event = fakeEvent('event-1')
  ...
  it('evicts expired entries oldest-first, respecting insertion order', () => {
    const clock = fakeClock()
    const sink = new DedupingEventSink({ retentionWindowMs: 100, now: clock.now })
    ...
    // Triggers a sweep as a side effect of offering a new event.
    sink.offer(fakeEvent('new-1'))
  ...
  it('evicts the single oldest entry once maxEntries is exceeded', () => {
    const sink = new DedupingEventSink({ maxEntries: 2 })
  ...
  it('defaults to a 5 minute retention window', () => {
    const clock = fakeClock()
    const sink = new DedupingEventSink({ now: clock.now })
    const event = fakeEvent('event-1')
